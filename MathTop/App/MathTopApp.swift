...
struct PracticeView: View {
    ...
    let onComplete: () -> Void
    var body: some View { NavigationStack { VStack(alignment: .leading, spacing: 24) { HStack { Text(lesson.title).font(.title2.bold()); Spacer(); Text("\(step + 1) / 3").font(.subheadline.bold()).foregroundStyle(.secondary) }; ProgressView(value: Double(step + 1), total: 3).tint(coral); Text(step == 0 ? "先观察，再选择你的捷径" : step == 1 ? "把数字拆开，答案会自己出现" : "能力解锁完成！").font(.title3.bold()).foregroundStyle(ink); Spacer(); VStack(spacing: 12) { Text(step < 2 ? (lesson.questions.first?.prompt ?? lesson.summary) : "你获得了：\(lesson.ability)") .font(.system(size: 28, weight: .bold, design: .rounded)).foregroundStyle(ink); if step < 2 { ForEach(lesson.questions.first?.choices ?? [], id: \.id) { choice in Button { answer = choice.id } label: { Text(choice.text).font(.title3.bold()).frame(maxWidth: .infinity).padding().background(answer == choice.id ? mint : Color.white).foregroundStyle(ink).clipShape(RoundedRectangle(cornerRadius: 14)) } } } else { Image(systemName: "bolt.circle.fill").font(.system(size: 70)).foregroundStyle(coral) } }; Spacer(); Button { if step < 2 { if let question = lesson.questions.first { store.recordAttempt(questionID: question.id, lessonID: lesson.id, correct: answer == question.answer) }; step += 1 } else { onComplete(); dismiss() } } label: { Text(step < 2 ? "确认答案" : "收下能力") .font(.headline).frame(maxWidth: .infinity).padding().background(ink).foregroundStyle(.white).clipShape(Capsule()) } }.padding(22).background(Color(red: 0.98, green: 0.98, blue: 0.96).ignoresSafeArea()).toolbar { ToolbarItem(placement: .topBarLeading) { Button("退出") { dismiss() } } } } }
}
