...
struct PracticeView: View {
    ...
    @State private var step = 0; @State private var answer = ""; @State private var submitted = false
    private var question: Question? { lesson.questions.indices.contains(step) ? lesson.questions[step] : nil }
    let onComplete: () -> Void
    var body: some View { NavigationStack { VStack(alignment: .leading, spacing: 24) { HStack { Text(lesson.title).font(.title2.bold()); Spacer(); Text("\(min(step + 1, max(lesson.questions.count, 1))) / \(max(lesson.questions.count, 1))").font(.subheadline.bold()).foregroundStyle(.secondary) }; ProgressView(value: Double(step + 1), total: Double(max(lesson.questions.count, 1))).tint(coral); Text(question == nil ? "能力解锁完成！" : submitted ? (answer == question?.answer ? "答对了！" : "再看一眼这个思路") : "先观察，再选择你的答案").font(.title3.bold()).foregroundStyle(ink); Spacer(); VStack(spacing: 12) { Text(question?.prompt ?? "你获得了：\(lesson.ability)").font(.system(size: 28, weight: .bold, design: .rounded)).foregroundStyle(ink); if let question { if submitted { Text(question.explanation).font(.body).foregroundStyle(ink.opacity(0.75)).padding().frame(maxWidth: .infinity, alignment: .leading).background(mint.opacity(0.35)).clipShape(RoundedRectangle(cornerRadius: 14)) } else { ForEach(question.choices, id: \.id) { choice in Button { answer = choice.id } label: { Text(choice.text).font(.title3.bold()).frame(maxWidth: .infinity).padding().background(answer == choice.id ? mint : Color.white).foregroundStyle(ink).clipShape(RoundedRectangle(cornerRadius: 14)) } } } } else { Image(systemName: "bolt.circle.fill").font(.system(size: 70)).foregroundStyle(coral) } }; Spacer(); Button { if let question { if !submitted { store.recordAttempt(questionID: question.id, lessonID: lesson.id, correct: answer == question.answer); submitted = true } else { answer = ""; submitted = false; step += 1 } } else { onComplete(); dismiss() } } label: { Text(question == nil ? "收下能力" : submitted ? "下一题" : "确认答案").font(.headline).frame(maxWidth: .infinity).padding().background(answer.isEmpty && question != nil && !submitted ? Color.gray : ink).foregroundStyle(.white).clipShape(Capsule()) }.disabled(answer.isEmpty && question != nil && !submitted) }.padding(22).background(Color(red: 0.98, green: 0.98, blue: 0.96).ignoresSafeArea()).toolbar { ToolbarItem(placement: .topBarLeading) { Button("退出") { dismiss() } } } } }
}
...
struct GrowthSummaryView: View {
    @EnvironmentObject private var store: LearningStore
    var body: some View { let summary = GrowthSummaryService.make(store: store); return List { Section("学习总览") { row("已解锁知识点", "\(summary.completed)/\(summary.total)"); row("累计练习题", "\(summary.attempts)"); row("待复习错题", "\(summary.incorrect)") }; Section("掌握率") { ProgressView(value: summary.mastery).tint(coral); Text("继续完成下一个知识点，掌握率会持续提升。").font(.caption).foregroundStyle(.secondary) } }.navigationTitle("成长摘要") }
    private func row(_ title: String, _ value: String) -> some View { HStack { Text(title); Spacer(); Text(value).font(.headline).foregroundStyle(coral) } }
}
