...
enum MathContent {
    ...
    private static let baseLessons: [Lesson] = [
        ...
        lesson("p-word-extra", "应用题拆解练习", "解决问题", .primary, "用数量关系解决生活问题"),
        lesson("j-challenge-real", "综合压轴训练", "建模推理", .junior, "方程、函数、几何与概率综合训练"),
    ...
    private static let questionBank: [String: [Question]] = [
        ...
        "j-counting": [q("j-counting-1", "抛一枚硬币正面朝上的概率？", ["0", "1/2", "1"], "1/2", "正反两种等可能结果。"), q("j-counting-2", "概率的取值范围？", ["0到1", "1到2", "任意数"], "0到1", "概率不小于 0 且不大于 1。"), q("j-counting-3", "袋中有 3 个红球、2 个白球，任取 1 个取到红球的概率？", ["2/5", "3/5", "1/2"], "3/5", "总球数 5 个，红球 3 个，概率为 3/5。"), q("j-counting-4", "同时掷两枚骰子，点数和为 2 的概率？", ["1/6", "1/12", "1/36"], "1/36", "只有 (1,1) 一种结果，共 36 种等可能结果。")]
        ...
        "p-word-extra": [
            q("p-word-extra-1", "一桶油连桶重 18 千克，用去一半油后连桶重 10 千克。空桶重多少千克？", ["2千克", "4千克", "8千克"], "2千克", "用去的一半油重 18-10=8 千克，原有油 16 千克，空桶重 18-16=2 千克。"),
            q("p-word-extra-2", "甲数比乙数多 25%，乙数比甲数少百分之几？", ["20%", "25%", "33.3%"], "20%", "设乙数为 100，甲数为 125；少的 25 占甲数的 25÷125=20%。"),
            q("p-word-extra-3", "一项工程甲单独做 12 天完成，乙单独做 18 天完成。合作 4 天后还剩几分之几？", ["4/9", "5/9", "7/9"], "4/9", "合作效率为 1/12+1/18=5/36，4 天完成 5/9，还剩 4/9。"),
            q("p-word-extra-4", "一辆车前 2 小时每小时行 60 千米，后 3 小时每小时行 80 千米。全程平均速度是多少？", ["70千米/时", "72千米/时", "75千米/时"], "72千米/时", "总路程 2×60+3×80=360 千米，总时间 5 小时，平均速度为 72 千米/时。"),
            q("p-word-extra-5", "一个分数的分子、分母同时加 2 后为 3/4，同时减 1 后为 2/3。原分数是？", ["5/7", "7/10", "8/11"], "7/10", "检验 7/10：同时加 2 得 9/12=3/4，同时减 1 得 6/9=2/3。"),
            q("p-word-extra-6", "某商品先提价 20%，再降价 20%，现价是原价的多少？", ["96%", "100%", "104%"], "96%", "设原价为 100，变化后为 100×1.2×0.8=96。"),
            q("p-word-extra-7", "把一根绳子剪成 5 段需要 12 分钟。按相同速度剪成 8 段需要多少分钟？", ["18分钟", "21分钟", "24分钟"], "21分钟", "剪成 5 段需剪 4 次，每次 3 分钟；8 段需剪 7 次，共 21 分钟。"),
            q("p-word-extra-8", "长方形长增加 20%，宽减少 20%，新面积比原面积怎样？", ["减少4%", "不变", "增加4%"], "减少4%", "面积倍率为 1.2×0.8=0.96，因此减少 4%。"),
            q("p-word-extra-9", "鸡兔同笼共有 35 个头、94 只脚。兔有多少只？", ["12只", "14只", "17只"], "12只", "若全是鸡有 70 只脚，多出的 24 只脚对应 12 只兔。"),
            q("p-word-extra-10", "浓度 20% 的盐水 300 克，蒸发多少克水后浓度变为 25%？", ["40克", "60克", "75克"], "60克", "盐有 60 克；浓度为 25% 时总重 60÷25%=240 克，蒸发 60 克。"),
            q("p-word-extra-11", "一列火车长 180 米，以每秒 20 米的速度通过长 420 米的隧道，需要多少秒？", ["21秒", "30秒", "39秒"], "30秒", "车尾离开隧道时车头共行 180+420=600 米，用时 600÷20=30 秒。"),
            q("p-word-extra-12", "某班男生人数的 1/4 与女生人数的 1/5 相等，男女共 45 人。男生有多少人？", ["20人", "24人", "25人"], "20人", "男生:女生=4:5，共 9 份；每份 5 人，男生 20 人。"),
            q("p-word-extra-13", "一个两位数，十位与个位数字之和为 11，交换数字后比原数大 27。原数是？", ["38", "47", "56"], "47", "交换后增加 27，个位比十位大 3；和为 11，所以两位数字为 4 和 7。"),
            q("p-word-extra-14", "从山脚到山顶每小时走 3 千米，原路返回每小时走 5 千米。往返平均速度是多少？", ["3.75千米/时", "4千米/时", "4.25千米/时"], "3.75千米/时", "设单程 15 千米，往返 30 千米，用时 5+3=8 小时，平均速度 3.75 千米/时。"),
            q("p-word-extra-15", "一个长方体的长、宽、高都扩大到原来的 2 倍，表面积和体积分别变为原来的多少倍？", ["4倍和8倍", "6倍和8倍", "8倍和8倍"], "4倍和8倍", "面积随长度倍率的平方变化，体积随长度倍率的立方变化。"),
            q("p-word-extra-16", "甲仓粮食比乙仓多 30 吨。若从甲仓运 10 吨到乙仓，两仓粮食比为 5:4。甲仓原有多少吨？", ["50吨", "60吨", "70吨"], "60吨", "设乙仓原有 x 吨，则 (x+20):(x+10)=5:4，解得 x=30，甲仓为 60 吨。"),
            q("p-word-extra-17", "一个圆的半径增加 50%，面积增加百分之几？", ["50%", "100%", "125%"], "125%", "新面积是原面积的 1.5²=2.25 倍，因此增加 125%。"),
            q("p-word-extra-18", "有 10 级台阶，每次可上 1 级或 2 级。要使步数最少且恰有 2 次上 1 级，共需走几步？", ["5步", "6步", "7步"], "6步", "两次上 1 级共走 2 级，余下 8 级用 4 次上 2 级，共 6 步。"),
            q("p-word-extra-19", "一本书第一天读全书的 1/3，第二天读余下的 40%，还剩 72 页。全书多少页？", ["150页", "180页", "200页"], "180页", "第一天后剩 2/3，第二天后剩其中 60%，即全书的 2/5；72÷2/5=180。"),
            q("p-word-extra-20", "一个水池装有进水管和排水管。只开进水管 6 小时装满，只开排水管 9 小时排空。空池同时开两管，几小时装满？", ["12小时", "18小时", "24小时"], "18小时", "实际效率为 1/6-1/9=1/18，因此 18 小时装满。")
        ],
        "j-challenge-real": [
            q("jcr-1", "若 2x-3=5x+6，则 x=?", ["-3", "3", "-1"], "-3", "移项得 -3x=9，所以 x=-3。"), q("jcr-2", "方程 x²-5x+6=0 的两根是？", ["2和3", "-2和-3", "1和6"], "2和3", "分解为 (x-2)(x-3)=0。"), q("jcr-3", "函数 y=-2x+5 中，y 随 x 如何变化？", ["增大", "减小", "不变"], "减小", "一次项系数 -2<0，函数递减。"), q("jcr-4", "抛物线 y=(x-2)²-3 的顶点是？", ["(2,-3)", "(-2,-3)", "(2,3)"], "(2,-3)", "顶点式 y=(x-h)²+k 的顶点为 (h,k)。"), q("jcr-5", "若 a+b=7，ab=10，则 a²+b²=?", ["29", "39", "49"], "29", "a²+b²=(a+b)²-2ab=49-20。"),
            q("jcr-6", "直角三角形斜边 13，一直角边 5，另一直角边？", ["8", "12", "18"], "12", "由勾股定理得 √(169-25)=12。"), q("jcr-7", "相似三角形边长比 2:3，面积比是？", ["2:3", "4:9", "8:27"], "4:9", "相似图形面积比等于相似比的平方。"), q("jcr-8", "圆心角 90°、半径 4 的扇形面积？", ["4π", "8π", "16π"], "4π", "90/360×π×4²=4π。"), q("jcr-9", "一组数 2,4,4,6,9 的中位数？", ["4", "5", "6"], "4", "排序后居中的第三个数是 4。"), q("jcr-10", "不透明袋中红球 2 个、蓝球 3 个，不放回连取两个均为红球的概率？", ["1/10", "1/5", "2/5"], "1/10", "2/5×1/4=1/10。"),
            q("jcr-11", "若 |x-2|=5，则 x=?", ["7或-3", "3或-7", "只有7"], "7或-3", "x-2=±5。"), q("jcr-12", "分式 1/(x-2) 有意义的条件？", ["x≠2", "x≠-2", "x>2"], "x≠2", "分母不能为零。"), q("jcr-13", "√12 化简为？", ["2√3", "3√2", "6√2"], "2√3", "√12=√(4×3)=2√3。"), q("jcr-14", "若一次函数经过 (0,2)、(2,6)，解析式？", ["y=2x+2", "y=x+2", "y=3x"], "y=2x+2", "斜率为 (6-2)/2=2，截距为2。"), q("jcr-15", "等腰三角形顶角 40°，底角各为？", ["40°", "70°", "80°"], "70°", "(180°-40°)÷2=70°。"),
            q("jcr-16", "矩形对角线长 10，一边长 6，另一边？", ["4", "8", "16"], "8", "由勾股定理 √(100-36)=8。"), q("jcr-17", "售价 240 元比进价提高 20%，进价？", ["192元", "200元", "288元"], "200元", "进价×1.2=240。"), q("jcr-18", "甲单独 6 天完成，乙单独 3 天完成，合作需？", ["1天", "2天", "3天"], "2天", "效率和为 1/6+1/3=1/2。"), q("jcr-19", "连续两个整数的平方差为 15，较小正整数？", ["6", "7", "8"], "7", "(n+1)²-n²=2n+1=15，n=7。"), q("jcr-20", "点 P(3,-4) 到原点距离？", ["5", "7", "25"], "5", "距离为 √(3²+(-4)²)=5。"),
            q("jcr-21", "若 x+y=8 且 xy=12，则 (x-y)²=?", ["16", "24", "40"], "16", "(x-y)²=(x+y)²-4xy=64-48。"), q("jcr-22", "关于 x 的方程 x²-4x+m=0 有两个相等实根，则 m=?", ["2", "4", "8"], "4", "判别式 16-4m=0。"), q("jcr-23", "点 A(-2,3) 关于 y 轴对称点是？", ["(2,3)", "(-2,-3)", "(3,-2)"], "(2,3)", "关于 y 轴对称时横坐标变号。"), q("jcr-24", "反比例函数 y=6/x 经过点 (3,?)", ["1", "2", "3"], "2", "代入 x=3 得 y=2。"), q("jcr-25", "若 3^(x+1)=27，则 x=?", ["1", "2", "3"], "2", "27=3³，所以 x+1=3。"),
